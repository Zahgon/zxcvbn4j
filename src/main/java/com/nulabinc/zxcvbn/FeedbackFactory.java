...
class FeedbackFactory {
    ...
    private static Feedback createSpatialMatchFeedback(Match match) {
        String warning = match.turns == 1 ? Feedback.SPATIAL_WARNING_STRAIGHT_ROWS_OF_KEYS : Feedback.SPATIAL_WARNING_SHORT_KEYBOARD_PATTERNS;
        return new Feedback(warning, Feedback.EXTRA_SUGGESTIONS_ADD_ANOTHER_WORD, Feedback.SPATIAL_SUGGESTIONS_USE_LONGER_KEYBOARD_PATTERN);
    }

    private static Feedback createRepeatMatchFeedback(Match match) {
        String warning = match.baseToken.length() == 1 ? Feedback.REPEAT_WARNING_LIKE_AAA : Feedback.REPEAT_WARNING_LIKE_ABCABCABC;
        return new Feedback(warning, Feedback.EXTRA_SUGGESTIONS_ADD_ANOTHER_WORD, Feedback.REPEAT_SUGGESTIONS_AVOID_REPEATED_WORDS);
    }
    ...
    private static Feedback createRegexMatchFeedback(Match match) {
        String warning = "recent_year".equals(match.regexName) ? Feedback.REGEX_WARNING_RECENT_YEARS : null;
        return new Feedback(warning, Feedback.EXTRA_SUGGESTIONS_ADD_ANOTHER_WORD, Feedback.REGEX_SUGGESTIONS_AVOID_RECENT_YEARS);
    }
