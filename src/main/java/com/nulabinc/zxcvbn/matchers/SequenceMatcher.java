...
public class SequenceMatcher extends BaseMatcher {
    ...
    private Match createSequenceMatch(CharSequence password, int startIndex, int endIndex, int codePointDiff) {
        if (!isValidSequenceLength(endIndex, startIndex, codePointDiff) || !isValidCodePointDiffValue(codePointDiff)) {
            return null;
        }
    ...
    private boolean isValidSequenceLength(int endIndex, int startIndex, int codePointDiff) {
        return (endIndex - startIndex) > MIN_VALID_SEQUENCE_LENGTH || Math.abs(codePointDiff) == MIN_VALID_SEQUENCE_LENGTH;
    }
