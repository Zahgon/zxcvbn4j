...
public class RepeatMatcher extends BaseMatcher {
    ...
    private java.util.regex.Matcher createRegionMatcher(Pattern pattern, CharSequence password, int start, int end) {
        java.util.regex.Matcher matcher = pattern.matcher(password);
        matcher.region(start, end);
        return matcher;
    }

    private ChosenMatch chooseMatch(java.util.regex.Matcher greedyMatcher, java.util.regex.Matcher lazyMatcher) {
    ...
    private Match createRepeatMatch(CharSequence baseToken, String matchResult, int start, int end) {
        ...
        return MatchFactory.createRepeatMatch(start, end, matchResult, wipeableBaseToken, baseAnalysis.getGuesses(), baseAnalysis.getSequence(), repeatCount);
    }
